...
This time we will use the `mutate()` function from the [dplyr](http://cran.r-project.org/web/packages/dplyr/index.html) package. 
...
with each observation having the Monday of the week for that observation. Check
that it is actually a date using the `str()` function.
...
When you have completed the activity, compare your results to the [solutions](03_advanced_graphics-solution.html#Date).
...
When you have completed the activity, compare your results to the [solutions](03_advanced_graphics-solution.html#graph).
...
When you have completed the activity, compare your results to the [solutions](03_advanced_graphics-solution.html#facet).
...
```{r}
early_w <- early %>% group_by(week) %>% summarize(count = nrow(.))
late_w  <- late  %>% group_by(week) %>% summarize(count = nrow(.))
mid_w   <- mid   %>% group_by(week) %>% summarize(count = nrow(.))
...
When you have completed the activity, compare your results to the [solutions](03_advanced_graphics-solution.html#subsetting).
