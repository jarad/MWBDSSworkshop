...
Subsetting a date works similar to subsetting a continuous (numeric) variable once you convert to comparison value to a `Date` using the `as.Date()` function.
...
```{r}
early_w = ddply(early, .(weekD), summarize, count = length(id))
late_w  = ddply(late,  .(weekD), summarize, count = length(id))
mid_w   = ddply(mid,   .(weekD), summarize, count = length(id))

g1 = ggplot(early_w, aes(x=weekD, y=count)) + geom_point()
g2 = ggplot(late_w,  aes(x=weekD, y=count)) + geom_point()
g3 = ggplot(mid_w,   aes(x=weekD, y=count)) + geom_point()
```
