...
GI_count <- GI %>%
  ...
  summarize(total = nrow(.))       # . in a placeholder for the dataset
...
the object `g`. Then you can add elements to the plot and if you don't assign
it, the plot will be shown. 

```{r}
g = ggplot(GI %>% 
             group_by(week) %>%
             summarize(coiunt = nrow(.)),
